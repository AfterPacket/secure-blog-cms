...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["install"])) {
    ...
    if (
        ...
        if (
            empty($adminUsername) ||
            !preg_match('/^[a-zA-Z0-9_-]{3,20}$/', $adminUsername)
        ) {
            $errors[] =
                "Username must be 3-20 characters (letters, numbers, underscores, dashes only)";
        }
            ...
            <form method="post" action="index.php?step=2" id="installForm">
                ...
                <fieldset style="border: none; margin-bottom: 30px;">
                    ...
                    <div class="form-group">
                        ...
                        <ul class="password-policy" id="install-password-policy" style="margin:4px 0;padding-left:18px;font-size:0.85rem;color:#888;">
                            <li id="inst-policy-length" data-regex=".{12,}">Minimum 12 characters</li>
                            <li id="inst-policy-uppercase" data-regex="[A-Z]">At least one uppercase letter</li>
                            <li id="inst-policy-lowercase" data-regex="[a-z]">At least one lowercase letter</li>
                            <li id="inst-policy-digit" data-regex="[0-9]">At least one digit</li>
                            <li id="inst-policy-special" data-regex="[^a-zA-Z0-9]">At least one special character</li>
                        </ul>
                        <div class="password-strength">
                            <div class="password-strength-bar" id="strengthBar"></div>
                        </div>
                        <div class="form-help">Must meet all requirements above</div>
                    </div>
    ...
    <script>
        ...
        if (passwordInput && strengthBar) {
            passwordInput.addEventListener('input', function() {
                ...
                const policyItems = document.querySelectorAll('#install-password-policy li');
                policyItems.forEach(function(item) {
                    var regex = new RegExp(item.getAttribute('data-regex'));
                    if (regex.test(password)) {
                        item.style.color = '#27ae60';
                    } else {
                        item.style.color = password.length > 0 ? '#e74c3c' : '#888';
                    }
                });
            });
        }
