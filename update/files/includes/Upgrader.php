...
class Upgrader
{
    ...
    public function performUpgrade($version, $downloadUrl, $checksum)
    {
        ...
        try {
            ...
            foreach ($manifestData["files"] as $fileRelativePath => $info) {
                ...
                // Verify hash - REQUIRED for all files, no exceptions
                if (!isset($info["sha256"]) || empty($info["sha256"]) || $info["sha256"] === "auto") {
                    throw new Exception("Integrity check required but no checksum provided for: " . $fileRelativePath);
                }
    ...
    {
        // Auto-upgrade has been disabled for security reasons.
        // Upgrades must be manually triggered and verified.
        return ["success" => false, "error" => "Auto-upgrade has been disabled for security. Please upgrade manually."];
    }

    /**
     * Perform upgrade using only the manifest URL - no user-provided URLs.
     */
    public function performUpgradeFromManifest($version)
    {
        ...
        if (!isset($manifest["files"]) || !is_array($manifest["files"])) {
            return ["success" => false, "error" => "Manifest does not contain a valid file list."];
        }

        foreach ($manifest["files"] as $filePath => $info) {
            if (!isset($info["sha256"]) || empty($info["sha256"])) {
                return ["success" => false, "error" => "Integrity checksum missing for: " . $filePath . ". Upgrade rejected for safety."];
            }
        }

        // Use only the base URL from the manifest
        $downloadUrl = isset($manifest["base"]) ? $manifest["base"] : "";
        $checksum = ""; // Individual file checksums are used

        return $this->performUpgrade($version, $downloadUrl, $checksum);
    }
