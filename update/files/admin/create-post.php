<?php
/**
 * Secure Blog CMS - Create Post Page
 * Secure post creation with CSRF protection and input validation
 */
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!$security->validateCSRFToken($csrfToken, "create_post_form")) {
        ...
        if (empty($formData["title"])) {
            ...
            empty($formData["post_password"])
        ) {
            $message = "Password cannot be empty for a protected post.";
            $messageType = "error";
        } else {
            // Create post
            $result = $storage->createPost($formData);
...
// Generate CSRF token
$csrfToken = $security->generateCSRFToken("create_post_form");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
selector: '#content',
            ...
            const form = contentArea ? contentArea.closest('form') : null;
    ...
    $category["name"],
                    ...
                    <div style="border: 1px solid #eee; padding: 15px; border-radius: 4px;">
                        ...
                        <div class="form-group" style="margin-bottom: 15px;">
                            ...
                            <select id="visibility" name="visibility">
                                <option value="public" <?php echo ($formData[
                                    "visibility"
                                ] ??
                                    "public") ===
                                "public"
                                ...
                                <option value="private" <?php echo ($formData[
                                    "visibility"
                                ] ??
                                    "") ===
                                "private"
                ...
                <div class="form-group">
                    ...
                    <select id="status" name="status" required>
                        <option value="draft" <?php echo ($formData["status"] ??
                            "draft") ===
                        "draft"
                            ? "selected"
                            : ""; ?>>
                        ...
                        <option value="published" <?php echo ($formData[
                            "status"
                        ] ??
                            "") ===
                        "published"
