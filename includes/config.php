 ...
 *
 * ⚠️  DO NOT OVERWRITE THIS FILE DURING UPDATES  ⚠️
 * This file contains site-specific settings (credentials, URLs, etc.).
 * When updating, always preserve your existing config.php.
 * The updater and deployment scripts should NEVER replace this file.
 */
...
define("SECURE_CMS_VERSION", "1.5.4");
...
define("ENABLE_CSP_HEADERS", true);
...
// Set to true ONLY if running behind Cloudflare or a trusted reverse proxy
// that sets X-Forwarded-For / CF-Connecting-IP headers.
...
// Security: Refuse to run if default placeholder credentials are still in place
if (ADMIN_USERNAME === "REPLACE_ME_USERNAME" || ADMIN_PASSWORD_HASH === "REPLACE_ME_PASSWORD_HASH") {
    if (file_exists(__DIR__ . "/../data/installed.lock")) {
        // Installed but still has placeholder creds - log warning but allow
    } else {
        // Not installed and has placeholder creds - this is expected, installer will set them
    }
}
