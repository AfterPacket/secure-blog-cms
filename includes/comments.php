...
class Comments
{
    ...
    public function addComment($postId, $data)
    {
        ...
        $clientIP = $this->security->getClientIPPublic() ?: ($_SERVER["REMOTE_ADDR"] ?? "unknown");
        if (!$this->security->checkRateLimit("comment_" . $clientIP, 3, 3600)) {
            return [
                "success" => false,
                "message" => "You are submitting comments too quickly. Please wait before trying again.",
            ];
        }
