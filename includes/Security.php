...
class Security
{
     ...
     * Only trusts proxy headers if TRUST_PROXY_HEADERS is explicitly enabled.
     */
    private function getClientIP()
    {
        // Only trust CF-Connecting-IP or X-Forwarded-For if explicitly configured
        // behind a trusted reverse proxy. Otherwise these headers can be spoofed.
        if (defined('TRUST_PROXY_HEADERS') && TRUST_PROXY_HEADERS) {
            if (!empty($_SERVER["HTTP_CF_CONNECTING_IP"])) {
                // Cloudflare provides the true client IP
    ...
    private function setSecurityHeaders()
    {
        ...
        // Version header disabled by default for security (reduces reconnaissance).
        // Enable only if needed for monitoring/debugging by setting SHOW_VERSION_HEADER to true.
        if (defined("SHOW_VERSION_HEADER") && SHOW_VERSION_HEADER) {
    ...
    public function validateCSRFToken($token, $formName = "default")
    {
        ...
        // Token is valid — invalidate after use to prevent replay attacks.
        // Reusable tokens are a CSRF risk: if an attacker obtains a token,
        // they can replay it multiple times within the token lifetime.
        unset($_SESSION["csrf_tokens"][$formName]);
        return true;
    }
    ...
    /**
     * Remove dangerous XSS attributes from HTML using DOM-based parsing.
     * Regex-based sanitization is inherently fragile; DOM parsing is more reliable.
     */
    private function removeXSSAttributes($html)
    {
        // Use DOMDocument for robust HTML sanitization when available
        if (class_exists('DOMDocument')) {
            ...
            if ($loaded) {
                ...
                // Collect nodes to remove first to avoid modifying DOM during iteration
                $attrsToRemove = [];
                for ($i = 0; $i < $onAttrs->length; $i++) {
                    $attr = $onAttrs->item($i);
                ...
                // Remove dangerous non-event attributes
                $dangerousAttrs = ['formaction'];
                foreach ($dangerousAttrs as $attrName) {
                ...
                // Sanitize href and src attributes: remove javascript:, vbscript:, data:text/html
                $urlAttrs = $xpath->query('//@href | //@src');
                $attrsToRemove = [];
                for ($i = 0; $i < $urlAttrs->length; $i++) {
                    $attr = $urlAttrs->item($i);
                    if (!$attr || !$attr->ownerElement) {
                        continue;
                    }
                    $valLower = strtolower(trim($attr->nodeValue));
                    if (
                        strpos($valLower, 'javascript:') === 0 ||
                        strpos($valLower, 'vbscript:') === 0 ||
                        strpos($valLower, 'data:text/html') === 0 ||
                        strpos($valLower, 'data:') === 0
                    ) {
                        $attrsToRemove[] = $attr;
                    }
                }
                ...
                // Remove <script>, <svg>, <math>, <iframe>, <object>, <embed> tags entirely
                $dangerousTags = ['script', 'svg', 'math', 'iframe', 'object', 'embed', 'applet', 'form', 'meta', 'link', 'base'];
                ...
                // Serialize back, stripping wrapping added by DOMDocument
                $result = $dom->saveHTML();
                // Remove XML declaration and outer html/body wrappers that DOMDocument may add
        ...
        // Fallback: regex-based sanitization if DOMDocument is unavailable
        // This is less robust but provides baseline protection

        // Remove ALL on* event handler attributes (covers hundreds of event types)
        $html = preg_replace('/\bon\w+\s*=\s*(?:["\'][^"\']*["\']|[^\s>]*)/i', '', $html);

        // Remove formaction attribute
        $html = preg_replace('/\bformaction\s*=\s*(?:["\'][^"\']*["\']|[^\s>]*)/i', '', $html);
        ...
        $html = preg_replace('/<(script|svg|math|iframe|object|embed|applet|form|meta|link|base)[^>]*>.*?<\/\1>/is', '', $html);
        $html = preg_replace('/<(script|svg|math|iframe|object|embed|applet|form|meta|link|base)[^>]*>/i', '', $html);
    ...
    /**
     * Get client IP address safely for use in rate limiting, logging, etc.
     * Respects TRUST_PROXY_HEADERS setting.
     * @return string
     */
    public function getClientIPPublic()
    {
