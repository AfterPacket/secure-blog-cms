...
class Security {
    ...
    public function sanitizeInput($input, $type = 'string') {
        ...
        switch ($type) {
            ...
            case 'int':
                ...
                $input = filter_var($input, FILTER_VALIDATE_INT) !== false ? (int)$input : 0;
                break;

            case 'float':
                $input = filter_var($input, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                $input = filter_var($input, FILTER_VALIDATE_FLOAT) !== false ? (float)$input : 0.0;
                break;
    ...
    private function removeXSSAttributes($html) {
        ...
        $dangerous = [
            ...
            'ondrop', 'onabort', 'oncanplay', 'oncanplaythrough', 'oncuechange',
            'ondurationchange', 'onemptied', 'onended', 'onloadeddata',
            'onloadedmetadata', 'onloadstart', 'onpause', 'onplay', 'onplaying',
            'onprogress', 'onratechange', 'onseeked', 'onseeking', 'onstalled',
            'onsuspend', 'ontimeupdate', 'onvolumechange', 'onwaiting',
            'ontoggle', 'formaction', 'javascript:', 'vbscript:', 'data:text/html'
        ];

        foreach ($dangerous as $attr) {
            $html = preg_replace('/' . preg_quote($attr, '/') . '\s*=\s*["\'][^"\']*["\']/i', '', $html);
            $html = preg_replace('/' . preg_quote($attr, '/') . '\s*=\s*[^"\'\s>]*/i', '', $html);
        }

        // Remove javascript: and data: protocols from href and src
        $html = preg_replace('/(<[^>]+)(href|src)\s*=\s*["\']?(javascript|data|vbscript):[^"\'\s>]*/i', '$1', $html);
     ...
     * Rate limiting
     */
    public function checkRateLimit($identifier, $maxAttempts = RATE_LIMIT_REQUESTS, $timeWindow = RATE_LIMIT_PERIOD) {
        $rateLimitFile = SESSIONS_DIR . '/ratelimit_' . hash('sha256', $identifier) . '.json';
    ...
    public function checkLoginAttempts($username) {
        $attemptsFile = SESSIONS_DIR . '/login_' . hash('sha256', $username) . '.json';
    ...
    public function recordFailedLogin($username) {
        ...
        if (file_exists($attemptsFile)) {
            ...
            if ($data['attempts'] >= MAX_LOGIN_ATTEMPTS) {
                ...
                $this->logSecurityEvent('Account locked due to failed login attempts', $username);
            }
        }
    ...
    public function authenticate($username, $password) {
        ...
        // Verify credentials
        if ($username === ADMIN_USERNAME && $this->verifyPassword($password, ADMIN_PASSWORD_HASH)) {
        ...
        if ($safeUser !== '') {
            ...
            if (is_file($userFile)) {
                $userData = json_decode((string)file_get_contents($userFile), true);
                if (is_array($userData) && !empty($userData['password_hash']) && $this->verifyPassword($password, (string)$userData['password_hash'])) {
                    $this->clearLoginAttempts($username);
                    session_regenerate_id(true);

                    $_SESSION['authenticated'] = true;
                    $_SESSION['user'] = (string)($userData['username'] ?? $safeUser);
                    $_SESSION['role'] = (string)($userData['role'] ?? 'author');
                    $_SESSION['login_time'] = time();
                    $this->setSessionFingerprint();

                    $this->logSecurityEvent('Successful login (file user)', $_SESSION['user']);
                    return ['success' => true, 'message' => 'Login successful'];
                }
            }
        }
    ...
    private function cleanOldSessions() {
        ...
        foreach ($files as $file) {
            if (is_file($file) && ($currentTime - filemtime($file)) > SESSION_LIFETIME) {
                @unlink($file);
            }
        }
    }
