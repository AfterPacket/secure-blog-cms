    ...
    if (!$security->validateCSRFToken($csrfToken, "upgrade_action")) {
        ...
        switch ($action) {
            ...
            case "perform_upgrade":
                $version = $security->getPostData("version", "alphanumeric", "");
                if (empty($version)) {
                    $message = "Version not specified";
                    $messageType = "error";
                } else {
                    $result = $upgrader->performUpgradeFromManifest($version);
                    $message = $result["message"] ?? ($result["error"] ?? "Upgrade failed");
                    $messageType = $result["success"] ? "success" : "error";
                    if ($result["success"]) {
                        $message .= " (Upgraded from " . ($result["old_version"] ?? "unknown") . " to " . ($result["new_version"] ?? $version) . ")";
                    }
                }
                break;

            case "toggle_auto_upgrade":
                $result = $upgrader->setAutoUpgrade(isset($_POST["auto_upgrade_enabled"]));
                $message = $result["error"] ?? "Auto-upgrade setting updated.";
                $messageType = $result["success"] ? "success" : "error";
                break;
        }
    }
